...
export function initStatusSettings() {
    ...
    currentSettings =
        getStatusSettings() ||
        JSON.parse(JSON.stringify(DEFAULT_STATUS_SETTINGS));
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                <div class="anima-compact-input">
                    ...
                    <input type="number" id="inject-depth" class="anima-input" 
                           placeholder="0" value="${injectionSettings.depth ?? 4}">
                </div>
                <div class="anima-compact-input">
...
export function refreshStatusPanel() {
    ...
    try {
        // 1. 获取全量消息以定位 ID
        const allMsgs = window.TavernHelper.getChatMessages(
            "0-{{lastMessageId}}",
            {
...
function initYamlEditor() {
    ...
    $("#btn-sync-status")
        ...
        .on("click", async function (e) {
            ...
            if (
                !confirm("确定要请求副API重新生成当前状态吗？\n这将消耗Token。")
            )
                return;

            $icon
                .removeClass("fa-cloud-arrow-down")
...
function initInjectionModule() {
    ...
    $("#btn-inject-save-card").on("click", async () => {
        ...
        // 2. 【修改】保存到全局 extensionSettings (不再写卡)
        // saveStatusSettings 会写入 extensionSettings，这正是我们要的“全局”效果
        saveStatusSettings(currentSettings);

        // 3. 应用到世界书 (Worldbook)
        // 即使没有角色卡，世界书逻辑通常是针对 "current" chat 的
        // 如果当前没聊天，这一步可能会被 syncStatusToWorldBook 内部拦截，这没问题
        try {
            await syncStatusToWorldBook();
            toastr.success("注入配置已保存 (全局) 并尝试应用");
        } catch (e) {
            console.error(e);
...
function initZodModule() {
    ...
    $btnConfirm.on("click", () => {
        ...
        exitScriptEdit();
        if (window.toastr)
            window.toastr.success("脚本已暂存 (记得保存到角色卡)");
    });
        ...
        .on("click", (e) => {
            ...
            try {
                if (window.TavernHelper) {
                    ...
                    // 为了避免日志刷屏，只显示部分
                    const preview =
                        JSON.stringify(realOldData).slice(0, 40) + "...";
                    log(
                        `环境就绪: 已加载基准数据 (oldData) [${preview}]`,
                        "info",
                    );
                } else {
                    log(
                        "警告: 未检测到 TavernHelper，oldData 将为空对象。",
                        "warn",
                    );
                }
            } catch (e) {
            ...
            try {
                if (mode === "ui") {
                    ...
                    try {
                        ...
                        const utils = {
                            val: (path, def) =>
                                window._.get(realOldData, path, def),
                            getVar: (name) => {
                                if (
                                    window.TavernHelper &&
                                    window.TavernHelper.getVariable
                                ) {
                                    return window.TavernHelper.getVariable(
                                        name,
                                    );
                                }
                                return null;
                        ...
                        if (
                            !userSchema ||
                            typeof userSchema.safeParse !== "function"
                        ) {
                            throw new Error(
                    ...
                    if (result.success) {
                        ...
                        log(
                            "最终数据: " + JSON.stringify(result.data, null, 2),
                            "info",
                        );
                    } else {
                        log("校验失败 ❌ 详细原因:", "error");
    ...
    $btnConfirm.on("click", async (e) => {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                setTimeout(
                    () =>
                        $("#status-yaml-content").removeClass(
                            "anima-input-active",
                        ),
                    300,
                );
        ...
        reader.onload = (ev) => {
            try {
                ...
                if (Array.isArray(json)) {
                    ...
                    if (window.toastr)
                        toastr.error("文件格式错误：内容必须是 JSON 数组");
                }
            } catch (err) {
                console.error(err);
                if (window.toastr)
        ...
        if (
            ...
            try {
                ...
                rawMsgs.forEach((msg) => {
                    ...
                    // 2. 判断是否跳过正则 (保持不变)
                    if (
                        regexSettings.skip_layer_zero &&
                        String(msg.message_id) === "0"
                    )
                        isSkipped = true;
                    if (regexSettings.regex_skip_user && isUser)
                        isSkipped = true;
