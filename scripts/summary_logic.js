    ...
    messages.forEach((msg, idx) => {
        ...
        // 🟢 3. User 消息处理逻辑 (受开关控制)
            ...
            if (regex_skip_user) {
                // 开关开启：跳过正则
                console.log(
                    `${logPrefix} -> 保留原文 (User Raw - Skipped by Setting)`,
                );
                isSkippedRegex = true;
            } else {
                // 开关关闭：执行正则
...
export async function requestSummaryFromAPI(
    ...
    for (const item of settings.summary_messages) {
        ...
        if (item.type === "prev_summaries") {
            ...
            if (count > 0) {
                const prevText = await getPreviousSummaries(
                    effectiveIndex,
                    count,
                );
                if (prevText) {
    ...
    try {
        ...
        if (!customRange) {
            ...
            if (wbInfo && wbInfo.maxEndId > currentMetaId) {
                ...
                if (window.toastr)
                    toastr.info(
                        `Anima: 进度已从世界书同步至 #${wbInfo.maxBatchId}`,
                    );
            }
        }
        ...
        if (lastSummarizedId === -1 && !customRange) {
            try {
                ...
                if (hiddenMsgs && hiddenMsgs.length > 0) {
                    const maxHiddenId =
                        hiddenMsgs[hiddenMsgs.length - 1].message_id;
        ...
        while (keepRunning) {
            ...
            if (customRange) {
                ...
                if (latestId === calcTargetEndId) {
                    const isUser =
                        latestMsg.is_user || latestMsg.role === "user";
                    if (!isUser) {
                        console.log(
            ...
            console.log(
                `[Anima] 批次 #${finalIndex} 完成。检查是否有更多待处理...`,
            );
        ...
        if (window.toastr)
            toastr.error("自动化总结出错，已停止: " + err.message);
    } finally {
        isSummarizing = false;
