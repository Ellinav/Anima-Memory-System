...
export function renderStrategyTable(settings) {
    ...
    if (
        ...
        Array.isArray(settings.period_config.events)
    ) {
        // 提取所有已配置事件的 Label
...
export async function renderUnifiedFileList() {
    ...
    const html = items
        ...
        <div class="anima-rag-file-item" style="border-left: 3px solid ${borderColor}; display:flex; justify-content:space-between; align-items:center; padding:8px 10px; margin-bottom:6px; border-radius:4px; background:rgba(0,0,0,0.2);">
            ...
            <button class="anima-btn danger small btn-delete-file" data-file="${escapeHtml(dbId)}" title="⚠️ 彻底删除此数据库文件 (物理删除)" style="padding:2px 6px; opacity:0.6;">
    ...
    // 5. 事件绑定 (只保留物理删除功能)
    container.find(".btn-delete-file").on("click", async function () {
        const file = $(this).data("file");
        if (
            !confirm(
                `⚠️ 严重警告：\n\n您即将彻底删除数据库 "${file}"。\n此操作将物理删除服务器上的文件，无法撤销！\n\n若只想取消关联，请点击上方的“关联”按钮进行取消勾选。`,
            )
        )
            return;

        const { deleteCollection } = await import("./rag_logic.js");
        const result = await deleteCollection(file);

        if (result && result.success) {
            // 删除成功后，自动从 Metadata 移除
            let nextChat = (getChatRagFiles() || []).filter((f) => f !== file);
            let nextKb = (getChatKbFiles() || []).filter((f) => f !== file);

            await saveChatRagFiles(nextChat);
            await saveChatKbFiles(nextKb);

            toastr.success(`已彻底删除: ${file}`);
            renderUnifiedFileList(); // 刷新列表
        }
    });
}
        ...
        settings.holidays.forEach((h, idx) => {
            ...
            if (isEditing) {
                $tr.find(".btn-save").click(() => {
                    ...
                    if (!date || !name)
                        return toastr.warning("日期和名称不能为空");

                    settings.holidays[idx] = {
                        date: date,
                        name: name,
                        range_before:
                            parseInt($tr.find(".h-before").val()) || 0,
...
function renderPeriodModal(settings) {
    ...
    const renderList = (optionalEditIdx = -1) => {
        ...
        tempEvents.forEach((ev, idx) => {
            ...
            const textStyle =
                "text-align:center; vertical-align:middle; color:#ddd;";
            ...
            if (isEditing) {
                $tr.find(".btn-save-row").click(() => {
                    ...
                    if (!label || !start)
                        return toastr.warning("标签和开始日期不能为空");
                    ...
                    tempEvents[idx] = {
                        ...
                        start_date: start,
                        cycle_length:
                            parseInt($tr.find(".p-cycle").val()) || 28,
                        duration: parseInt($tr.find(".p-dur").val()) || 5,
                        range_before:
                            parseInt($tr.find(".p-before").val()) || 0,
