    ...
    switch (op) {
        ...
        case "includes":
            ...
            if (typeof actual === "string")
                return actual.includes(String(target));
            return false;
        case "not_includes":
            if (Array.isArray(actual)) return !actual.includes(target);
            if (typeof actual === "string")
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            error: (jqXHR) => {
                ...
                console.error(
                    `[Anima Debug] ❌ Backend Error [${endpoint}]:`,
                    errMsg,
                );
...
export async function insertMemory(
    ...
    try {
        ...
        if (response && response.vectorId) {
            ...
            console.warn(
                "[Anima Debug] ⚠️ 后端返回成功但不包含 vectorId:",
                response,
            );
            return null;
        }
...
export async function queryDual({
    ...
    const betterMainId = cleanExtras.find(
        (extraId) =>
            extraId.includes(cleanMainId) &&
            extraId.length > cleanMainId.length,
    );
    ...
    if (pConfig.enabled !== false && Array.isArray(pConfig.events)) {
        pConfig.events.forEach((event) => {
            if (event.label)
                definedCyclicLabels.push(event.label.toLowerCase());
    ...
    const chatStrategyPayload = {
        ...
        steps: [
            ...
            },
            {
                type: "special",
                count:
                    activeHolidayTags.length > 0
                        ? stratConfig.special.count
                        : 0,
