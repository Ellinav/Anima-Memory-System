...
export async function saveSummaryBatchToWorldbook(
    ...
    try {
        ...
        if (!existing) {
            ...
            console.log(
                `[Anima] 目标世界书 ${wbName} 可能未加载，尝试自动创建/读取`,
            );
    ...
    if (targetEntry) {
        ...
        await window.TavernHelper.updateWorldbookWith(wbName, (entries) => {
            ...
            // 过滤掉旧 Batch
            e.extra.history = e.extra.history.filter(
                (h) => h.batch_id !== batchId,
            );
            // 推入新 Batch
    ...
    if (successIds.length > 0) {
        ...
        console.log(
            `[Anima] 已更新 ${successIds.length} 条记录的同步状态为 True`,
        );
    }
}
...
export async function deleteSummaryItem(wbName, entryUid, targetIndex) {
    try {
        ...
        if (targetEntry && targetEntry.extra && targetEntry.extra.source_file) {
            ...
            if (window.toastr)
                toastr.success(`后端向量 #${targetIndex} 已清理`);
        }
    } catch (e) {
    ...
    await window.TavernHelper.updateWorldbookWith(wbName, (entries) => {
        ...
        if (Array.isArray(e.extra.history)) {
            e.extra.history = e.extra.history.filter((h) => {
                ...
                const currentId =
                    h.unique_id !== undefined ? h.unique_id : h.index;
...
export async function updateKnowledgeEntry(content) {
    ...
    if (exists) {
        ...
        await window.TavernHelper.createWorldbookEntries(wbName, [
            newEntryStub,
        ]);
...
export async function triggerVectorUpdate(index) {
    ...
    if (!targetEntry || !historyItem) {
        console.warn(
            `[Anima] 未找到 Index ${index} 的世界书条目，跳过向量化。`,
        );
        return;
    }
    ...
    const finalSettings = {
        ...
        depth:
            (settings.depth !== undefined
                ? settings.depth
                : globalSettings.depth) ?? 9999,
        order:
            (settings.order !== undefined
                ? settings.order
                : globalSettings.order) ?? 100,
    };
...
export async function syncRagSettingsToWorldbook() {
    ...
    const chatSettings = allSettings.injection || {}; // 聊天注入配置
