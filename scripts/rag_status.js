...
export async function showVectorStatusModal() {
    ...
    const modalHtml = `
        <div style="margin-bottom:15px; padding-bottom:15px; border-bottom:1px solid rgba(255,255,255,0.1); display:flex; justify-content:space-between; align-items:center;">
            ...
            <div style="display:flex; gap:10px;">
                <button id="anima-rag-btn-sync-dirty" class="anima-btn small primary">
                    <i class="fa-solid fa-cloud-arrow-up"></i> 一键同步
                </button>
                <button id="anima-rag-btn-rebuild-all" class="anima-btn small danger">
                    <i class="fa-solid fa-dumpster-fire"></i> 一键重建
                </button>
                <button id="anima-rag-btn-refresh-list" class="anima-btn small secondary">
    ...
    $("#anima-rag-btn-sync-dirty").on("click", async function () {
        ...
        for (let i = 0; i < total; i++) {
            ...
            try {
                // A. 获取文本
                const text = await getSummaryTextFromEntry(
                    item.uid,
                    item.uniqueId,
                );
                        ...
                        await window.TavernHelper.updateWorldbookWith(
                            ...
                            (entries) => {
                                ...
                                if (
                                    e &&
                                    e.extra &&
                                    Array.isArray(e.extra.history)
                                ) {
                                    const h = e.extra.history.find(
                                        (x) =>
                                            String(x.unique_id || x.index) ===
    ...
    $("#anima-rag-btn-rebuild-all").on("click", async function () {
        ...
        const $btn = $(this);
        $btn.prop("disabled", true).html(
            '<i class="fa-solid fa-circle-notch fa-spin"></i> 处理中...',
        );
        $("#anima-rag-btn-refresh-list").prop("disabled", true);
        ...
        for (let i = 0; i < total; i++) {
            ...
            try {
                ...
                if (text) {
                    ...
                    failCount++;
                    console.warn(
                        `[Rebuild] Skipped empty text for #${item.uniqueId}`,
                    );
                }
            } catch (err) {
        ...
        $btn.prop("disabled", false).html(
            '<i class="fa-solid fa-dumpster-fire"></i> 重建向量库',
        );
        $("#anima-rag-btn-refresh-list").prop("disabled", false);
    ...
    const html = pageItems
        .map((item) => {
            ...
            const vectorBadge = item.isVectorized
                ? `<span class="anima-tag-badge status-badge" style="background:rgba(74, 222, 128, 0.2); border-color:#22c55e; color:#4ade80;"><i class="fa-solid fa-check"></i> 已向量化</span>`
                : `<span class="anima-tag-badge status-badge" style="background:rgba(248, 113, 113, 0.2); border-color:#ef4444; color:#f87171;"><i class="fa-solid fa-clock"></i> 未向量化</span>`;
        ...
        <div class="anima-history-entry" 
            ...
            <div class="anima-history-header">
                <div class="anima-history-meta" style="flex:1;">
                    <span style="color:#fbbf24; font-weight:bold; margin-right:8px;">#${item.uniqueId}</span>
                    ${vectorBadge}
                    <span style="color:#666; font-size:12px; margin-left:8px;">楼层 ${item.range_start}-${item.range_end}</span>
                </div>
                ...
                <div class="anima-history-actions" style="display:flex; align-items:center; gap:5px;">
                    <div class="actions-normal">
                        <button class="anima-btn small secondary btn-refresh-vector" title="刷新向量 (重新读取文本并向量化)">
                            <i class="fa-solid fa-database"></i>向量化
                        </button>
