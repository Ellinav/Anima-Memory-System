...
import { clearRagEntry, clearKnowledgeEntry } from "./scripts/worldbook_api.js";
import {
    getStatusSettings,
...
(function () {
    ...
    function bindGlobalEvents() {
        ...
        if (context && context.eventSource) {
            ...
            const triggerAutomationCheck = (source, customDelay = 1000) => {
                ...
                if (getIsSummarizing()) {
                    console.log(
                        `[Anima] Ignored ${source}: Task already running.`,
                    );
                    return;
                }
            ...
            context.eventSource.on("generation_started", (type, arg1, arg2) => {
                ...
                // C. 取消上一轮的倒计时 (如果有)
                console.log(
                    "[Anima] 🚨 检测到新请求，重置状态标志并取消倒计时。",
                );
                cancelStatusTimer();
            });
            context.eventSource.on("generation_stopped", () => {
                console.log(
                    "[Anima] 🛑 用户手动取消了生成 (Generation Stopped)",
                );
            ...
            context.eventSource.on("generation_ended", async () => {
                ...
                try {
                    ...
                    // 【新增】生成结束后，如果状态面板是打开的，顺便刷新一下 UI
                    // 延时一点点，确保 TavernHelper 变量写入完成
                    setTimeout(() => refreshStatusPanel(), 500);
                } catch (e) {
                    console.error(
                        "[Anima] Post-generation status update failed:",
                        e,
                    );
                }
            });
    ...
    function getExtensionBasePath() {
        ...
        for (const script of scripts) {
            if (
                script.src &&
                script.src.includes(`/${targetFolder}/index.js`)
            ) {
