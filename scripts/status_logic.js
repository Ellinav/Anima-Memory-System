...
export function getStatusSettings() {
    ...
    if (charId) {
        ...
        // --- 读取 注入配置 ---
        const cardInjection = getSettingsFromCharacterCard(
            "anima_rag_injection",
        );
        if (cardInjection) {
            finalSettings.injection_settings = cardInjection;
        } else {
            // 你的默认值里写的是 injection_settings，注意命名一致
            finalSettings.injection_settings = structuredClone(
                DEFAULT_STATUS_SETTINGS.injection_settings,
            );
        }
...
async function getIncrementalChatContext(targetMsgId, baseMsgId, contextData) {
    ...
    // 2. 截取片段 (限制最大深度 20)
    if (targetIndex - startIndex > 20)
        startIndex = Math.max(0, targetIndex - 20);
    ...
    incrementalMsgs.forEach((msg) => {
        ...
        // 再次 trim() 确保没有首尾空白
        if (content.trim())
            chatContext += `${displayName}: ${content.trim()}\n\n`;
    });
    ...
    return {
        ...
        range: {
            start: incrementalMsgs[0]?.message_id ?? "Start",
                ...
                incrementalMsgs[incrementalMsgs.length - 1]?.message_id ??
                "End",
...
export async function triggerStatusUpdate(targetMsgId) {
    ...
    try {
        ...
        if (!payload) {
            console.warn("[Anima] Invalid JSON response");
            return;
        }
        ...
        console.error("[Anima] Update failed:", e);
        if (window.toastr) window.toastr.error("状态更新异常: " + e.message);
        return false; // 🟢 改动：发生异常，返回 false
    }
}
...
// 防线检查
function checkReplyIntegrity(content) {
    ...
    try {
        ...
        try {
            ...
            if (targetEventSource) {
                const newAnimaData =
                    fullStatusData.anima_data || fullStatusData;
                ...
                console.log(
                    "[Anima] 📡 已成功广播事件: ANIMA_VARIABLE_UPDATE_ENDED",
                );
            } else {
                console.warn(
...
export async function handleStatusUpdate() {
    ...
    const isPanelEnabled = updateConfig.panel_enabled === true; // 只有显式为 true 才倒计时

    // 6. 分支逻辑
    if (isPanelEnabled) {
        // A. 开启面板 -> 通知 UI 检查按钮显隐(作为未同步的提示) -> 显示倒计时
        const checkEvent = new CustomEvent("anima:status_updated");
...
function createCountdownUI(seconds, onConfirm, onCancel) {
    ...
    <style>
        .anima-floating-panel {
            ...
            background: var(--smart-background); border: 1px solid var(--smart-border-color);
...
export function initStatusMacro() {
    ...
    window.TavernHelper.registerMacroLike(
        ...
        (context, match, capturedId) => {
            ...
            if (renderContext.anima_data)
                renderContext = renderContext.anima_data;
            ...
            try {
                // 1. 变量替换
                let finalOutput = template.replace(
                    /{{\s*([^\s}]+)\s*}}/g,
                    (m, path) => {
                        // A. 特殊硬编码路径处理
    ...
    window.TavernHelper.registerMacroLike(
        ...
        (context, match, keyPath) => {
            ...
            if (keyPath && keyPath.trim()) {
                ...
                // B. 全量模式 (保持原有逻辑)
                return Object.keys(baseData).length > 0
                    ? objectToYaml(baseData)
                    : "{}";
            }
        },
    );
    ...
    try {
        ...
        if (matchedIndex === -1 && targetText.length > 50) {
            ...
            if (pFirst.startsWith(shortTarget)) matchedIndex = 0;
                ...
                for (
                    let i = 0;
                    i < charData.data.alternate_greetings.length;
                    i++
                ) {
                    const pAlt = normalizeText(
                        processMacros(
                            charData.data.alternate_greetings[i] || "",
                        ),
                    );
        ...
        if (matchedIndex === -1) {
            console.log(
                "[Anima] 未匹配到已知开场白 (可能是自定义内容或宏解析差异)",
            );
            // Debug: 打印出来对比
